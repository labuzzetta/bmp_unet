...
ids_total = [s.split("_")[7] for s in glob.glob("/work/LAS/zhuz-lab/clabuzze/terrace_test_sample_500/hillshade_clipped_IA_halfFishs/terrace/*.tif")]

metric_f = FScore()
metric_i = IOUScore()
# returns a compiled model
# identical to the previous one
model = load_model('/work/LAS/zhuz-lab/clabuzze/model_wrr_te_nolidar.h5', custom_objects={'image_dice_coeff':image_dice_coeff, 'image_dice_coeff_round':image_dice_coeff_round, 'global_dice_coef':global_dice_coef, 'global_dice_loss':global_dice_loss, 'cldice':cldice, 'combined_loss':combined_loss, 'matthews_correlation':matthews_correlation, 'recall_m':recall_m, 'precision_m':precision_m, 'f1-score':metric_f, 'iou_score':metric_i})

for id in ids_total:
    
    x_pred_batch = []

    img = cv2.imread('/work/LAS/zhuz-lab/clabuzze/terrace_test_sample_500/hillshade_clipped_IA_halfFishs/terrace/fishGrid_{}_lidar_hs_1m.tif'.format(id))
    img = cv2.resize(img, (input_size, input_size))
    img0 = img
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    img = np.expand_dims(img, axis=2)

    img2 = cv2.imread('/work/LAS/zhuz-lab/clabuzze/terrace_test_sample_500/ortho_clipped_IA_halfFishs_2007_2010/terrace/fishGrid_{}_ortho_2007_2010_cir.tif'.format(id))
    img2 = cv2.resize(img2, (input_size, input_size))

    mask = cv2.imread('/work/LAS/zhuz-lab/clabuzze/terrace_test_sample_500/te_raster_clipped_IA_halfFishs/fishGrid_{}.tif'.format(id))
    ...
    img_stack = img2 #np.concatenate((img, img2), axis=2)
    ...
    #cv2.imwrite('/work/LAS/zhuz-lab/clabuzze/preprocess_classification_BMP_centroid/PRO_valid/te_truth_{}_{}.png'.format(id, round(dice_val,2)), vis)
